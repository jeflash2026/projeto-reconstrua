...
const Card = ({ c }: { c: CartaoCaso }): ReactElement => (
  <div className={`caso-card caso-urg-${c.urgencia}`}>
    <div className="caso-top">
      ...
      <span className={`cc-tag ${URG[c.urgencia]}`}>
        {c.urgencia === 'alta' ? 'Urgente' : c.urgencia === 'media' ? 'Atenção' : 'Normal'}
      </span>
    </div>
    <div className="caso-tags">
      <span className="caso-status">{c.status}</span>
      {c.grauConfianca ? (
        <span className={`cc-tag ${CONF[c.grauConfianca]}`}>Confiança {c.grauConfianca}</span>
      ) : (
        <span className="cc-tag cc-tag-informacao">Em apuração</span>
      )}
    </div>
    <p className="caso-hipotese">
      {c.principalHipotese ?? 'Ainda sem tese sustentável — reunir os fatos básicos.'}
    </p>
    {c.proximaAcao ? (
      <p className="caso-acao">
        <span>Próxima ação:</span> {c.proximaAcao}
      </p>
    ) : null}
    <dl className="caso-kv">
      <div>
        <dt>Docs pendentes</dt>
        <dd>{c.documentosPendentes.length === 0 ? 'nenhum' : c.documentosPendentes.join(', ')}</dd>
      </div>
      <div>
        <dt>Tempo parado</dt>
        <dd>{tempoParado(c.tempoParadoMs)}</dd>
      </div>
      <div>
        <dt>Dossiê</dt>
        <dd>{c.dossieDisponivel ? 'disponível' : 'em apuração'}</dd>
      </div>
      <div>
        <dt>Missão</dt>
        <dd className="mono">{c.missionId ?? '—'}</dd>
      </div>
      <div>
        <dt>Responsável</dt>
        <dd>{c.advogadoResponsavel ?? 'a distribuir'}</dd>
      </div>
    </dl>
    <Link href={c.href} className="caso-cta">
      Abrir Dossiê →
    </Link>
  </div>
);
  ...
  return (
    <>
      ...
      <p className="page-sub">
        Cada card é um caso. Comece pelo Dossiê — a AHRI já preparou o parecer.
      </p>
      {data.casos.length === 0 ? (
        <div className="cc-empty">
          ...
          <p>
            Nenhum caso na sua fila agora. Assim que um cliente chegar, a AHRI monta o dossiê e ele
            aparece aqui.
          </p>
        </div>
      ) : (
