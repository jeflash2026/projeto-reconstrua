    ...
    <ul className="lab-know-list">
      {g.itens.map((f, i) => (
        <li key={i}>
          ...
          <Link href={`/clientes/${encodeURIComponent(f.clienteId)}`} className="lab-know-cli">
            {f.clienteNome}
          </Link>
          <span className="lab-know-meta">
            {f.confianca} · {f.origem}
          </span>
...
const ConhecimentoPage = async (): Promise<ReactElement> => {
  const data = await getJson<{ categorias: CategoriaConhecimento[] }>(
    '/admin/inteligencia/conhecimento',
  );
  if (!data) {
    return (
      <>
  ...
  return (
    <>
      ...
      <p className="page-sub">
        O que a AHRI aprendeu conversando — apenas fatos estruturados, nunca mensagens.
      </p>
      {data.categorias.length === 0 ? (
        <div className="cc-empty">
          ...
          <p>
            Nenhum fato aprendido ainda. A cada conversa, a AHRI estrutura o que descobre e mostra
            aqui.
          </p>
        </div>
      ) : (
        <div className="lab-know-grid">
          {data.categorias.map((g) => (
            <Grupo key={g.factKey} g={g} />
          ))}
        </div>
      )}
    </>
