...
const AtribuirAdvogado = ({ prontos, advogados }: { prontos: ClientePronto[]; advogados: { id: string; name: string }[] }): ReactElement => {
  ...
  const atribuir = (missionId: string): void => {
    ...
    if (advogadoId === '') { setResultado((r) => ({ ...r, [missionId]: 'Escolha um advogado primeiro.' })); return; }
    iniciar(async () => {
      const r = await assignCase(missionId, advogadoId, 'administrador');
      if (r === null) { setResultado((s) => ({ ...s, [missionId]: 'Falha na atribuição — tente novamente.' })); return; }
      const aviso = r.aviso === 'enviado'
  ...
  return (
    <div className="card">
      <table className="table">
        <thead>
          <tr><th>Cliente</th><th>Status</th><th>Advogado</th><th></th></tr>
        </thead>
        <tbody>
          {prontos.map((c) => (
            <tr key={c.missionId}>
              <td>
                ...
                <div className="mono" style={{ fontSize: 12, opacity: 0.7 }}>{c.chatId.split('@')[0]}</div>
              </td>
              <td>
                <span className="badge accent">{c.status === 'AGUARDANDO_10_DIAS' ? 'prazo de 10 dias em curso' : 'aguardando sócio'}</span>
              </td>
              <td>
                <select
                  className="sol-input"
                  value={escolhas[c.missionId] ?? ''}
                  onChange={(e) => { setEscolhas((s) => ({ ...s, [c.missionId]: e.target.value })); }}
                >
              ...
              <td>
                <button className="btn" onClick={() => { atribuir(c.missionId); }} disabled={pendente}>
                  {pendente ? 'Encaminhando…' : 'Encaminhar'}
                </button>
                {resultado[c.missionId] ? <div style={{ fontSize: 12, marginTop: 4 }}>{resultado[c.missionId]}</div> : null}
              </td>
            </tr>
          ))}
        </tbody>
