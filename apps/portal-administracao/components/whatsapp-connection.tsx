...
const WhatsAppConnection = (): ReactElement => {
  ...
  const gerarQr = async (): Promise<void> => {
    ...
    if (fresh && (fresh.base64 || fresh.pairingCode)) setQr(fresh);
      ...
      setError(
        'Não foi possível gerar o QR agora. Clique em "Atualizar Status" e tente novamente.',
      );
    setBusy(false);
    void refresh();
  };
  ...
  return (
    <>
      ...
      <div
        className="card"
        style={{ marginBottom: 16, borderColor: connected ? 'var(--accent)' : undefined }}
      >
        ...
        {connected ? (
          <p>
            Número: <strong className="mono">{fmtNumber(status?.live?.number ?? '')}</strong>
          </p>
        ) : (
          <>
            <p>
              Estado: <span className="badge dim">{status?.live?.state ?? 'sem conexão'}</span>
            </p>
            <p style={{ color: 'var(--text-dim)', fontSize: 13 }}>
              Gere o QR e leia com o WhatsApp oficial (+{status?.officialNumber ?? '—'}) para
              conectar.
            </p>
          </>
        )}
      ...
      <div className="card">
        ...
        {src ? (
          <div style={{ marginTop: 16, textAlign: 'center' }}>
            ...
            {qr?.pairingCode ? (
              <p style={{ marginTop: 8 }}>
                Código de pareamento: <strong className="mono">{qr.pairingCode}</strong>
              </p>
            ) : null}
