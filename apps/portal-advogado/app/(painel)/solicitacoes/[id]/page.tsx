...
function tituloDoPasso(h: SolicitacaoHistorico): string {
  ...
  if (h.de === h.para && h.nota === 'mensagem enviada ao cliente')
    return 'AHRI notificou o cliente';
  if (h.de === h.para && h.nota === 'lembrete automático enviado') return 'AHRI lembrou o cliente';
  switch (h.para) {
    case 'AWAITING_CONFIRMATION':
      return 'AHRI confirmando com o cliente';
    case 'RECEIVED':
      return 'Documento recebido';
    case 'REOPENED':
      return 'Solicitação reaberta pelo advogado';
    case 'CANCELLED':
      return 'Solicitação cancelada';
    case 'PENDING':
      return 'Voltou a aguardar o cliente';
  }
}

const DetalhePage = async ({ params }: { params: { id: string } }): Promise<ReactElement> => {
  const s = await getJson<Solicitacao>(
    `/advogado/document-requests/${encodeURIComponent(params.id)}`,
  );
  if (!s) {
    return (
      <>
        ...
        <div className="error-box">
          Solicitação não encontrada (ou a operação está indisponível).
        </div>
        <Link
          href="/solicitacoes"
          className="sol-btn"
      ...
      <div className="sol-detalhe-head">
        <div>
          ...
          <div className="sol-detalhe-meta">
            <span className="mono" title="Cliente">
              {s.clientId.replace('@s.whatsapp.net', '')}
            </span>
            <span className="mono" title="Processo/Caso">
              caso {s.caseId}
            </span>
            <span title="Advogado responsável">{s.requestedBy}</span>
            {s.priority === 'alta' ? (
              <span className="sol-tag sol-tag-alta">PRIORIDADE ALTA</span>
            ) : null}
            <span className={prazo.vencida ? 'sol-vencida' : ''}>{prazo.texto}</span>
          </div>
        </div>
        <span className={`sol-tag sol-tag-grande ${STATUS_CLASSE[s.status]}`}>
          {STATUS_AMIGAVEL[s.status]}
        </span>
      </div>

      {s.optionalMessage ? (
        <div className="sol-mensagem-original">Sua mensagem ao cliente: “{s.optionalMessage}”</div>
      ) : null}
        ...
        <ol className="sol-timeline">
          {s.history.map((h, i) => (
            <li key={i} className="sol-passo">
              <span className="sol-passo-icone" aria-hidden>
                {h.de === null ? '📝' : h.de === h.para ? '💬' : ICONE[h.para]}
              </span>
              <div className="sol-passo-corpo">
                ...
                <div className="sol-passo-meta">
                  {new Date(h.at).toLocaleString('pt-BR')} · por {h.por}
                </div>
              </div>
            </li>
          ))}
