...
function aplica(f: Filtro, s: Solicitacao): boolean {
  switch (f) {
    case 'todas':
      return true;
    case 'pendentes':
      return s.status === 'PENDING' || s.status === 'AWAITING_CONFIRMATION';
    case 'urgentes':
      return ehUrgente(s);
    case 'reabertas':
...
const Card = ({ s }: { s: Solicitacao }): ReactElement => {
  ...
  return (
    <Link
      href={`/solicitacoes/${s.requestId}`}
      className={`sol-card ${STATUS_CLASSE[s.status]}${ehUrgente(s) ? ' sol-urgente' : ''}`}
    >
      ...
      <div className="sol-card-meta">
        <span title="Cliente" className="mono">
          {s.clientId.replace('@s.whatsapp.net', '')}
        </span>
        <span title="Caso" className="mono">
          caso {s.caseId}
        </span>
        <span title="Responsável atual">
          com: <b>{responsavelAtual(s)}</b>
        </span>
      </div>
    ...
    <>
        ...
        <div className="sol-filtros" role="tablist">
          {FILTROS.map((f) => (
            <button
              ...
              className={`sol-filtro${filtro === f.id ? ' ativo' : ''}`}
              onClick={() => {
                setFiltro(f.id);
              }}
            >
      ...
      {visiveis.length === 0 ? (
        <div className="sol-vazio">
          <div className="sol-vazio-icone" aria-hidden>
            {busca || filtro !== 'todas' ? '🔍' : '📄'}
          </div>
          <p>
            {busca || filtro !== 'todas'
          ...
          {!busca && filtro === 'todas' ? (
            <Link href="/solicitacoes/nova" className="sol-cta">
              Solicitar documento
            </Link>
          ) : null}
        </div>
