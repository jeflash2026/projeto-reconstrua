...
import { emptySnapshot, type HumanRole, type MissionSnapshot } from '../executive-brain/mission-snapshot.js';
...
class FakeIdentityMap implements MissionIdentityMap {
  ...
  load(chatId: string): Promise<MissionIdentity | null> { return Promise.resolve(this.map.get(chatId) ?? null); }
...
class FakeMemoryStore implements MemoryStore {
  ...
  load(chatId: string): Promise<ClientMemory | null> { return Promise.resolve(this.map.get(chatId) ?? null); }
  save(): Promise<void> { return Promise.resolve(); }
  all(): Promise<readonly ClientMemory[]> { return Promise.resolve([...this.map.values()]); }
}
class FakeSnapshotPort implements MissionSnapshotPort {
  constructor(private readonly map: Map<string, MissionSnapshot>) {}
  load(missionId: string): Promise<MissionSnapshot | null> { return Promise.resolve(this.map.get(missionId) ?? null); }
}
class FakeWorkflowStore implements WorkflowProgressStore {
  constructor(private readonly map: Map<string, MissionProgress>) {}
  load(missionId: string): Promise<MissionProgress | null> { return Promise.resolve(this.map.get(missionId) ?? null); }
  save(): Promise<void> { return Promise.resolve(); }
  all(): Promise<readonly MissionProgress[]> { return Promise.resolve([...this.map.values()]); }
}
class FakeSchedulerStore implements SchedulerStore {
  constructor(private readonly tasks: readonly ScheduledTask[]) {}
  save(): Promise<void> { return Promise.resolve(); }
  byId(id: string): Promise<ScheduledTask | null> { return Promise.resolve(this.tasks.find((t) => t.id === id) ?? null); }
  due(now: Date): Promise<readonly ScheduledTask[]> {
    return Promise.resolve(this.tasks.filter((t) => t.status === 'pending' && t.dueAt.getTime() <= now.getTime()));
  }
  pendingCount(): Promise<number> { return Promise.resolve(this.tasks.filter((t) => t.status === 'pending').length); }
  all(): Promise<readonly ScheduledTask[]> { return Promise.resolve(this.tasks); }
}
class FakeHandoffStore implements HandoffStore {
  constructor(private readonly tasks: readonly HandoffTask[]) {}
  save(): Promise<void> { return Promise.resolve(); }
  byId(id: string): Promise<HandoffTask | null> { return Promise.resolve(this.tasks.find((t) => t.id === id) ?? null); }
...
class FakeAssignmentStore implements AssignmentStore {
  ...
  save(): Promise<void> { return Promise.resolve(); }
  byMission(missionId: string): Promise<CaseAssignment | null> { return Promise.resolve(this.list.find((a) => a.missionId === missionId) ?? null); }
  byAdvogado(advogadoId: string): Promise<readonly CaseAssignment[]> { return Promise.resolve(this.list.filter((a) => a.advogadoId === advogadoId)); }
}
class FakeStaffStore implements StaffStore {
  constructor(private readonly list: readonly StaffMember[]) {}
  save(): Promise<void> { return Promise.resolve(); }
  byId(id: string): Promise<StaffMember | null> { return Promise.resolve(this.list.find((m) => m.id === id) ?? null); }
  byRole(role: StaffRole): Promise<readonly StaffMember[]> { return Promise.resolve(this.list.filter((m) => m.role === role)); }
  all(): Promise<readonly StaffMember[]> { return Promise.resolve(this.list); }
}
class FakeJuridicalStore implements JuridicalWorkStore {
  constructor(private readonly list: readonly JuridicalEntry[]) {}
  save(): Promise<void> { return Promise.resolve(); }
  byId(id: string): Promise<JuridicalEntry | null> { return Promise.resolve(this.list.find((e) => e.id === id) ?? null); }
  byAdvogado(advogadoId: string): Promise<readonly JuridicalEntry[]> { return Promise.resolve(this.list.filter((e) => e.advogadoId === advogadoId)); }
  byMission(missionId: string): Promise<readonly JuridicalEntry[]> { return Promise.resolve(this.list.filter((e) => e.missionId === missionId)); }
}
...
function identity(over: Partial<MissionIdentity>): MissionIdentity {
  return {
    chatId: 'c1', personId: 'p1', clienteId: 'cli-1', missionId: 'm1', caseId: 'case-1',
    processId: 'proc-1', latestTruthId: null, latestStateId: null, latestStageId: null,
    lastDocumentId: null, lastEventId: null, ...over,
  };
}
...
function snapshot(over: Partial<MissionSnapshot>): MissionSnapshot {
  return { ...emptySnapshot('m1'), stageCode: 'ANALISE', stateCode: 'EM_ANALISE', truthEstablished: true, ...over };
}
...
function fullSeed(over: { snapshots?: MissionSnapshot[] } = {}): Seed {
  return {
    ...
    progress: [{ missionId: 'm1', steps: ['documento_recebido', 'documento_reconhecido'], updatedAt: NOW }],
    tasks: [
      { id: 't1', chatId: 'c1', missionId: 'm1', kind: 'remind_client', dueAt: DUE, note: null, createdAt: NOW, status: 'pending' },
      { id: 't2', chatId: 'c1', missionId: 'm1', kind: 'follow_deadline', dueAt: NOW, note: null, createdAt: NOW, status: 'fired' },
    ],
    handoffs: [
      { id: 'h1', role: 'operador', reasonCode: 'DOC', missionId: 'm1', chatId: 'c1', operationalRuleRef: 'RO-1', fundamento: 'x', createdAt: NOW, status: 'open' },
    ],
    assignments: [{ missionId: 'm1', advogadoId: 'adv-1', assignedBy: 'admin', assignedAt: NOW }],
    staff: [{ id: 'adv-1', role: 'advogado', name: 'Dr X', email: null, active: true, createdAt: NOW, updatedAt: NOW }],
    juridical: [
      { id: 'j1', advogadoId: 'adv-1', missionId: 'm1', kind: 'numero_processo', text: '0001234-55.2026', dueAt: null, attachmentRef: null, done: true, createdAt: NOW },
      { id: 'j2', advogadoId: 'adv-1', missionId: 'm1', kind: 'prazo', text: 'contestação', dueAt: DUE, attachmentRef: null, done: false, createdAt: NOW },
    ],
  };
}
...
describe('ALIRProjectionBuilder · composição completa', () => {
  it('monta o Aggregate a partir das fontes reais', async () => {
    const { alir, metrics } = await new ALIRProjectionBuilder(sources(fullSeed())).compose('c1', { now: NOW });
    ...
    expect(alir.operational.missao.progresso).toEqual(['documento_recebido', 'documento_reconhecido']);
    expect(alir.operational.missao.acompanhamento).toEqual({ pendentes: 1, proximoVencimentoAt: DUE });
    ...
    expect(alir.operational.operacao.responsaveis).toEqual([{ id: 'adv-1', role: 'advogado', name: 'Dr X' }]);
    ...
    expect(alir.healthScore).toEqual({ score: 50, band: 'YELLOW', reasons: ['escalonamento humano aberto', 'documentos pendentes'] });

    // observabilidade (Regra 6)
    expect(metrics.contentHash).toMatch(/^[0-9a-f]{8}$/);
    expect(metrics.sourcesConsulted).toEqual(
      expect.arrayContaining(['identities', 'client-memory', 'mission-snapshot', 'workflow', 'scheduler', 'handoff', 'assignments', 'staff', 'juridical']),
    );
    expect(metrics.fieldsReconstructed).toEqual(expect.arrayContaining(['operational.missao.stateCode', 'healthScore', 'contentHash']));
    expect(metrics.fieldsUnavailable).toEqual(expect.arrayContaining(['operational.documentos.enviados', 'operational.timeline', 'operational.proximaAcao', 'extensions.pericia']));
...
describe('ALIRProjectionBuilder · cliente desconhecido', () => {
  it('sem identidade → ALIR vazio consistente, chatId como identidade provisória', async () => {
    const { alir, metrics } = await new ALIRProjectionBuilder(sources({})).compose('novo@c.us', { now: NOW });
...
describe('ALIRProjectionBuilder · composição parcial (Regra 5)', () => {
  it('groups=[CORE] não consulta fontes operacionais nem computa agregados', async () => {
    const { alir, metrics } = await new ALIRProjectionBuilder(sources(fullSeed())).compose('c1', { now: NOW, groups: ['CORE'] });
    expect(alir.core.pessoa.personId).toBe('p1');
...
describe('ALIRProjectionBuilder · hash estável (Regras 4/6)', () => {
  it('conteúdo igual → mesmo hash mesmo com projectedAt diferente; conteúdo diferente → hash diferente', async () => {
    ...
    const c = await new ALIRProjectionBuilder(sources({ ...changed, memories: [memory({ documentsPending: ['HISCON', 'CTPS'] })] })).compose('c1', { now: NOW });
...
describe('ALIRProjectionBuilder · alinhamento ao contrato (B-1)', () => {
  it('todo campo reportado nas métricas existe no registry ALIR_FIELDS', async () => {
    ...
    const { metrics } = await new ALIRProjectionBuilder(sources(fullSeed())).compose('c1', { now: NOW });
