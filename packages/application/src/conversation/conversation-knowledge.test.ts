...
function fatosMap(ctx: ConversationContextView) {
  return Object.fromEntries(
    aprenderDaConversa(ctx, CATALOGO_CONSIGNADO_INSS).map((f) => [f.factKey, f]),
  );
}
...
describe('Conversation Knowledge · o diálogo consignado do decreto', () => {
  ...
  it('nunca guarda perguntas nem respostas literais como conhecimento', () => {
    const resumo = resumoDoConhecimento(
      aprenderDaConversa(contexto(dialogoConsignado(), 'Sim.'), CATALOGO_CONSIGNADO_INSS),
    );
    expect(resumo).toBe(
      'problema_principal=descontos_nao_reconhecidos; tempo_do_problema=mais_de_2_anos; beneficio=aposentadoria; multiplos_bancos=true',
    );
  ...
  it('hiscon mencionado e interesse em revisão também são aprendidos', () => {
    const entries = [entry('inbound', 'eu tenho o hiscon aqui e quero a revisão dos contratos', 1)];
    const fatos = fatosMap(
      contexto(entries, 'eu tenho o hiscon aqui e quero a revisão dos contratos'),
    );
...
describe('Conversation Knowledge · conduz a conversa (Memory + Knowledge)', () => {
  ...
  it('motor GENÉRICO: trocar o catálogo troca o domínio (nenhum acoplamento)', () => {
    const catalogoOutroProduto: CatalogoDeConhecimento = [
      {
        factKey: 'porte_da_empresa',
        detectar: (r) => (/pequena empresa|mei\b/.test(r) ? { valor: 'pequena' } : null),
      },
    ];
    const ctx = contexto(
      [entry('inbound', 'tenho uma pequena empresa', 1)],
