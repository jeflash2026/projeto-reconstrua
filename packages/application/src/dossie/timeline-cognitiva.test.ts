...
import {
  resumirCaso,
  ordenarCasos,
  type ResumoCasoInputs,
  type CartaoCaso,
} from './painel-advogado.js';

function tlInputs(over: Partial<TimelineCognitivaInputs> = {}): TimelineCognitivaInputs {
  return {
    conversaIniciadaEm: new Date('2026-07-18T10:00:00Z'),
    totalMensagens: 6,
    beneficio: 'aposentadoria',
    fatosAprendidos: ['beneficio=aposentadoria', 'problema_principal=cartao_rmc'],
    documentos: [
      { label: 'HISCON', em: new Date('2026-07-18T11:00:00Z'), reconhecidoComo: 'hiscon' },
    ],
    contratos: ['contrato Banco X'],
    raciocinio: {
      totalHipoteses: 2,
      principal: 'EST-CONSIG-CARTAO-RMC-001',
      fatosDaPrincipal: ['problema_principal=cartao_rmc'],
    },
    decisao: {
      strategyRef: 'EST-CONSIG-CARTAO-RMC-001',
      confianca: 'alta',
      em: new Date('2026-07-18T11:05:00Z'),
    },
    missao: {
      missionId: 'M-1',
      criadaEm: new Date('2026-07-18T11:06:00Z'),
      advogado: 'dra. ana',
      recebidaEm: new Date('2026-07-18T11:07:00Z'),
    },
...
describe('13A · Timeline Cognitiva — a história do caso', () => {
  ...
  it('passos COGNITIVOS expõem os fatos utilizados (expansível); técnicos não', () => {
    const tl = montarTimelineCognitiva(tlInputs());
    expect(tl.find((i) => i.categoria === 'knowledge')?.fatosUtilizados).toContain(
      'problema_principal=cartao_rmc',
    );
    expect(tl.find((i) => i.categoria === 'mind')?.fatosUtilizados).toContain(
      'problema_principal=cartao_rmc',
    );
  ...
  it('só emite passos com EVIDÊNCIA — sem documento/decisão, não inventa', () => {
    const tl = montarTimelineCognitiva(
      tlInputs({
        documentos: [],
        contratos: [],
        raciocinio: null,
  ...
  it('encerramento e feedback entram quando ocorrem (append-only)', () => {
    const tl = montarTimelineCognitiva(
      tlInputs({
        encerradoEm: new Date('2026-07-19T09:00:00Z'),
        feedback: { em: new Date('2026-07-19T09:01:00Z'), decisao: 'confirmada' },
      }),
    );
        ...
        hipoteses: [
          {
            ...
            hipotese: 'Cartão RMC vendido como empréstimo',
            confianca: 'alta',
            prioridade: 70,
            justificativa: 'x',
            fundamento: 'y',
