...
describe('13A · Inteligência — biblioteca de estratégias', () => {
  it('traduz condições em texto legível', () => {
    expect(
      descreverCondicao({
        fact: 'problema_principal',
        op: 'eq',
        value: 'descontos_nao_reconhecidos',
      }),
    ).toBe('Problema principal é descontos_nao_reconhecidos');
    expect(
      descreverCondicao({ fact: 'beneficio', op: 'in', value: ['aposentadoria', 'pensao'] }),
    ).toContain('Benefício ∈ {aposentadoria, pensao}');
  });
