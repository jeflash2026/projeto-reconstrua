...
import type { AppendResult, StreamId, StreamType, ExpectedVersion } from '../../event-store/index.js';
...
class CaptureAppender implements EventAppender {
  ...
  append(_st: StreamType, _sid: StreamId, _ev: ExpectedVersion, events: readonly UncommittedEvent[]): Promise<AppendResult> {
...
function intent(over: Partial<MissionUseCaseIntent> = {}): MissionUseCaseIntent {
  return {
    useCase: 'CloseMission', references: [], decisor: 'AHRI', tipo: 'Decisão Operacional Automatizada',
    fundamento: 'encerramento', operationalRuleRef: 'RO-STOP-CONCLUDED-001', ...over,
  };
}
...
function ctx(intentIn: MissionUseCaseIntent): MissionContext {
  const facts: MissionFacts = { chatId: '5511999@c', senderId: '5511999@c', messageId: 'corr-turn-99', perceptKind: 'text', text: 'pode encerrar', mediaRef: null, fileName: null, mimeType: null, occurredAt: NOW };
  const identity = { ...emptyIdentity('5511999@c'), missionId: MISSION_UUID, clienteId: 'CLI-1', latestTruthId: TRUTH_UUID };
  return { intent: intentIn, facts, identity, now: NOW };
}

function run() {
  const appender = new CaptureAppender();
  const uc = new CloseMissionUseCase({ appender, uuid: new SeqUuid(), clock: new TestClock(), config: { ahriResponsibleId: 'dra. ana' } });
  return { appender, uc };
}
...
describe('12A · o evento de encerramento é enriquecido na ORIGEM', () => {
  it('com decisão estratégica (10C): carrega decisionId/strategyRef/confidence + identidade', async () => {
    ...
    const outcome = await uc.execute(ctx(intent({ strategicDecision: { decisionId: 'dec-abc12345', strategyRef: 'EST-CONSIG-REVISAO-001', confidence: 'alta', decisionReason: 'venceu' } })));
  ...
  it('sem decisão estratégica: não fabrica campos (só o que existe)', async () => {
    ...
    const payload = appender.payloads[0]?.[0] ?? {};
    expect(payload).toMatchObject({ missionId: MISSION_UUID, terminalState: 'ENCERRADA', cliente: 'CLI-1', correlationId: 'corr-turn-99' });
  ...
  it('não altera a lógica: continua exigindo Missão e Verdade de origem', async () => {
    const appender = new CaptureAppender();
    const uc = new CloseMissionUseCase({ appender, uuid: new SeqUuid(), clock: new TestClock(), config: { ahriResponsibleId: 'x' } });
    const semTruth: MissionContext = { ...ctx(intent()), identity: { ...emptyIdentity('c'), missionId: MISSION_UUID, latestTruthId: null } };
