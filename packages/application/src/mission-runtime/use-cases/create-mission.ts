...
// ─────────────────────────────────────────────────────────────────────────────
import {
  BeneficiaryPersonRef,
  InitialOperationalResponsibleRef,
  Mission,
  MissionId,
...
export class CreateMissionUseCase implements MissionUseCase {
  ...
  async execute(ctx: MissionContext): Promise<UseCaseOutcome> {
    if (ctx.identity.missionId !== null) {
      return skippedOutcome(this.name, this.streamType, ctx.identity.missionId, {
    ...
    if (ctx.identity.personId === null) {
      return failedOutcome(
        this.name,
        this.streamType,
        'pré-condição ausente: Pessoa beneficiária (INV-17)',
      );
    }
    ...
    const result = Mission.create({
      ...
      initialResponsible: InitialOperationalResponsibleRef.fromString(
