...
// ─────────────────────────────────────────────────────────────────────────────
import { DocumentAggregate, DocumentId, DocumentRecognitionResponsibleRef, MissionRef } from '@reconstrua/domain';
...
export class RecognizeDocumentUseCase implements MissionUseCase {
  ...
  async execute(ctx: MissionContext): Promise<UseCaseOutcome> {
    if (ctx.identity.missionId === null) {
      return failedOutcome(this.name, this.streamType, 'pré-condição ausente: Missão para incorporar o documento (INV-D08)');
    }
    const documentId = this.deps.uuid.next();
    const contentReference = ctx.facts.fileName ?? ctx.facts.mediaRef ?? ctx.facts.text ?? 'documento percebido';
    const result = DocumentAggregate.recognize({
      ...
      recognizedBy: DocumentRecognitionResponsibleRef.fromString(this.deps.config.ahriResponsibleId),
    });
    ...
    return successOutcome(this.name, this.streamType, documentId, appended, { lastDocumentId: documentId });
  }
}
