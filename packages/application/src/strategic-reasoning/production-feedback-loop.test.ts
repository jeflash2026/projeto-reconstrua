...
function auto(over: Partial<EncerramentoAutomatico>): EncerramentoAutomatico {
  return {
    missionId: 'M-1', decisionId: 'dec-abc12345', correlationId: 'corr-1', cliente: '5511999@c', advogado: 'dra. ana',
    data: new Date(Date.UTC(2026, 6, 10)), strategyRef: 'EST-CONSIG-REVISAO-001', confianca: 'alta',
    documentosRecebidos: ['HISCON'], documentosFaltantes: [], tempoAteDecisaoMs: 60_000, fatosAprendidos: ['beneficio=aposentadoria'],
    ...over,
  };
}
...
describe('11C · o encerramento gera o AtendimentoEncerrado automaticamente', () => {
  it('auto-preenche tudo que o sistema já tem; humano fornece só 3 campos', () => {
    const a = montarAtendimentoEncerrado(
      auto({ documentosFaltantes: ['extrato da RMC / cartão consignado'], fatosAprendidos: ['problema_principal=descontos_nao_reconhecidos'] }),
      { decisaoAdvogado: 'corrigida', estrategiaCorreta: 'EST-CONSIG-CARTAO-RMC-001', motivoCorrecao: 'era cartão RMC' },
    );
    // Automático:
  ...
  it('AUDITORIA: referencia missionId, decisionId, correlationId, cliente, data, advogado', () => {
    ...
    expect(a.auditoria).toEqual({
      missionId: 'M-1', decisionId: 'dec-abc12345', correlationId: 'corr-1',
      cliente: '5511999@c', data: new Date(Date.UTC(2026, 6, 10)), advogado: 'dra. ana',
    });
...
describe('11C · encerrar → persistir → o painel reflete imediatamente', () => {
  it('o laço completo alimenta o painel a cada encerramento', async () => {
    ...
    await loop.registrarEncerramento(auto({ missionId: 'M-1', tempoAteDecisaoMs: 40_000 }), { decisaoAdvogado: 'confirmada' });
    ...
    await loop.registrarEncerramento(
      auto({ missionId: 'M-2', tempoAteDecisaoMs: 120_000, documentosFaltantes: ['extrato da RMC / cartão consignado'], fatosDificeis: ['problema_principal'] }),
      { decisaoAdvogado: 'corrigida', estrategiaCorreta: 'EST-CONSIG-CARTAO-RMC-001', motivoCorrecao: 'RMC' },
    );
    ...
    expect(painel.estrategiasMaisUtilizadas[0]).toEqual({ chave: 'EST-CONSIG-REVISAO-001', ocorrencias: 2 });
    expect(painel.estrategiasMaisCorrigidas[0]).toMatchObject({ ref: 'EST-CONSIG-REVISAO-001', correcoes: 1 });
  ...
  it('HISTÓRICO MENSAL: agrupa por mês da data de auditoria', async () => {
    const store = new InMemoryAtendimentoStore();
    const loop = new ProductionFeedbackLoop(store);
    await loop.registrarEncerramento(auto({ missionId: 'J1', data: new Date(Date.UTC(2026, 5, 15)) }), { decisaoAdvogado: 'confirmada' });
    await loop.registrarEncerramento(auto({ missionId: 'J2', data: new Date(Date.UTC(2026, 5, 20)), confianca: 'media' }), { decisaoAdvogado: 'corrigida', estrategiaCorreta: 'EST-CONSIG-JUROS-001' });
    await loop.registrarEncerramento(auto({ missionId: 'L1', data: new Date(Date.UTC(2026, 6, 3)) }), { decisaoAdvogado: 'confirmada' });
  ...
  it('não recalcula: o painel só reflete o que está PERSISTIDO no store', async () => {
    const store = new InMemoryAtendimentoStore();
    const loop = new ProductionFeedbackLoop(store);
    const registrado = await loop.registrarEncerramento(auto({}), { decisaoAdvogado: 'confirmada' });
