...
const sessions = {
  getOrOpen: () =>
    Promise.resolve({ chatId: CHAT, turns: 0, lastInboundAt: null, lastOutboundAt: null }),
...
async function estado(
  ...
  const provider = criarMissaoProvider(
    snapshots(snapshot),
    clientes(status, missionId),
    new TestClock(),
    onboarding,
  );
  const view = await new ConversationContextRuntime(
    sessions,
    memory,
    {},
    undefined,
...
describe('Decreto · a MISSÃO ATIVA + Jornada 1 transicionam o estado da conversa', () => {
  ...
  it('ONBOARDING_DOCUMENTAL: missão ativa + documentação inicial INCOMPLETA', async () => {
    expect(
      await estado(
        snap({ caseExists: true, stateCode: 'ABERTA' }),
  ...
  it('contabilidade AUSENTE ou com falha ⇒ fail-closed: continua ONBOARDING', async () => {
    expect(await estado(snap({ caseExists: true }), 'COLETANDO_DOCUMENTOS', 'M-1', null)).toBe(
      'ONBOARDING_DOCUMENTAL',
    );
    const quebrada: OnboardingCompletude = {
      estaCompleto: () => Promise.reject(new Error('down')),
    };
    expect(await estado(snap({ caseExists: true }), 'COLETANDO_DOCUMENTOS', 'M-1', quebrada)).toBe(
      'ONBOARDING_DOCUMENTAL',
    );
  });

  it('CLIENTE: missão ativa + status do cliente VENDIDO', async () => {
    expect(await estado(snap({ caseExists: true }), 'VENDIDO', 'M-1', jornada1(true))).toBe(
      'CLIENTE',
    );
  });
  ...
  it('POS_ATENDIMENTO: missão ativa com estado terminal ENCERRADA (do Runtime)', async () => {
    expect(
      await estado(
        snap({ caseExists: true, stateCode: 'ENCERRADA' }),
        'EM_PROCESSO',
        'M-1',
        jornada1(true),
      ),
  ...
  it('a JORNADA COMPLETA do decreto muda a POLÍTICA e o OBJETIVO da missão', async () => {
    const jornada: Array<
      [MissionSnapshot | null, ClienteStatus, string | null, OnboardingCompletude | null]
    > = [
    ...
    const esperado = [
      { missao: 'LEAD', objetivo: 'Converter Lead', substitui: true },
      {
        ...
        objetivo: 'Completar a Documentação Inicial',
        substitui: true,
      },
      {
        missao: 'ANALISE_ADMINISTRATIVA',
        objetivo: 'Acompanhar a Análise Administrativa',
        substitui: false,
      },
