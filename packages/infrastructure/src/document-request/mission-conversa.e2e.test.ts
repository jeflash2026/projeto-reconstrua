...
import type {
  ConversationIntent, ConversationMemoryRuntime, MissionSnapshotPort,
  PendenciaDocumentalProvider, SessionRuntime, StoredEvent,
} from '@reconstrua/application';
import {
  ConversationContextRuntime, DocumentRequestRuntime, ObservabilityRuntime,
  PromptBuilderRuntime, emptySnapshot,
} from '@reconstrua/application';
...
function harness() {
  ...
  const inner: MissionSnapshotPort = { load: (chatId) => Promise.resolve({ ...emptySnapshot(chatId), caseExists: true }) };
  ...
  const sessions = { getOrOpen: () => Promise.resolve({ chatId: CHAT, turns: 3, lastInboundAt: null, lastOutboundAt: null }) } as unknown as SessionRuntime;
  const enviadas: string[] = [];
  const outbounds: string[] = [];
  const memory = {
    recent: () => Promise.resolve([]),
    recentOutboundTexts: () => Promise.resolve([]),
    recordOutbound: (_c: string, texto: string) => { outbounds.push(texto); return Promise.resolve(); },
  } as unknown as ConversationMemoryRuntime;
  const gateway = { sendText: (_c: string, texto: string) => { enviadas.push(texto); return Promise.resolve({ providerMessageId: `wa-${String(enviadas.length)}`, sentAt: NOW }); } };
  const contexto = new ConversationContextRuntime(sessions, memory, {}, undefined, () => Promise.resolve('CLIENTE'), pendencia);
  const builder = new PromptBuilderRuntime(8);
  const observability = new ObservabilityRuntime();
  const subscriber = new DocumentArrivalSubscriber({ store, runtime, gateway: gateway as never, observability, clock: new TestClock() });
  const comunicador = new DocumentRequestComunicador({ gateway: gateway as never, memory, runtime, nomeDoCliente: () => Promise.resolve('Jessé'), observability, clock: new TestClock() });
  return { store, runtime, snapshots, contexto, builder, subscriber, comunicador, enviadas, outbounds };
}

const intent: ConversationIntent = {
  id: 'i1', chatId: CHAT, directive: 'speak', speechAct: 'explain', topic: 'acompanhamento',
  references: [], urgency: 'normal', operationalRuleRef: 'RO-X', fundamento: 'f', timingHintMs: null, formedAt: NOW,
};
...
const NOVA = { requestId: '00000000-0000-4000-8000-0000000000e1', caseId: CASO, clientId: CHAT, lawyerId: 'ADV-1', documentName: 'Procuração', requestedBy: 'Dr. João Silva', createdAt: NOW } as const;
...
describe('15C-3 · associação INTELIGENTE (Parte 2)', () => {
  it('várias pendências + arquivo que casa com UMA ⇒ associa por IA', async () => {
    const h = harness();
    await h.runtime.criar(NOVA);
    await h.runtime.criar({ ...NOVA, requestId: '00000000-0000-4000-8000-0000000000e2', documentName: 'Extrato Bancário' });
  ...
  it('várias pendências + arquivo AMBÍGUO ⇒ AWAITING + pergunta ao cliente (received só depois)', async () => {
    ...
    expect((await h.store.porId('00000000-0000-4000-8000-0000000000e2'))?.status).toBe('AWAITING_CONFIRMATION');
