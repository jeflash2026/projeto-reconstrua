...
const PROFESSION =
  /(?:sou|trabalho como|minha profiss(?:ã|a)o (?:é|e))\s+([a-zà-ú]+(?:\s+[a-zà-ú]+)?)/i;
const CITY = /(?:moro em|sou de|aqui em)\s+([a-zà-ú]+(?:\s+[a-zà-ú]+)?)/i;
const FAMILY =
  /\b(meu pai|minha m(?:ã|a)e|meu filho|minha filha|minha esposa|meu marido|meu irm(?:ã|a)o|minha irm(?:ã|a))\b/i;
...
export class PatternAttributeExtractor implements MemoryAttributeExtractorPort {
  extract(text: string): Promise<readonly ProposedAttribute[]> {
    ...
    if (profession?.[1] && !name)
      proposed.push({ key: 'profession', value: profession[1].trim(), confidence: 0.6 });
    const family = FAMILY.exec(text);
    if (family?.[1])
      proposed.push({ key: 'family', value: family[1].toLowerCase(), confidence: 0.8 });
