...
import {
  ...
  type EncerramentoResolver,
} from './mission-closure-feedback-subscriber.js';
...
function harness(over: { resolver?: EncerramentoResolver; loop?: ProductionFeedbackLoop } = {}) {
  ...
  const sub = new MissionClosureFeedbackSubscriber({
    loop,
    resolver: over.resolver ?? defaultEncerramentoResolver,
    observability,
    uuid: new SeqUuid(),
    clock: new TestClock(),
  });
...
describe('GO-LIVE 11D · o hook dispara no encerramento e alimenta o painel', () => {
  it('ENCERRADA ⇒ registrarEncerramento ⇒ store persistido ⇒ painel atualizado', async () => {
    ...
    // Painel vazio antes.
    expect(
      montarPainelDoArquiteto(ESTRATEGIAS_CONSIGNADO_INSS, await store.listar()).totalAtendimentos,
    ).toBe(0);
    ...
    expect(persistidos[0]?.auditoria).toMatchObject({
      missionId: 'M-1',
      decisionId: 'dec-abc12345',
      correlationId: 'corr-1',
      cliente: '5511999@c',
...
describe('GO-LIVE 11D · falha isolada — o encerramento NÃO falha se o feedback falhar', () => {
  it('registrarEncerramento lançando ⇒ handle NÃO propaga; observabilidade registra o erro', async () => {
    ...
    const registrouFalha = errSpy.mock.calls.some(
      (c) => c[0] === 'feedback-loop' && String(c[1]).includes('result=fail'),
    );
...
describe('GO-LIVE 11D · resolver default usa exclusivamente dados existentes', () => {
  it('extrai do payload; strategyRef cai na RO quando ausente', () => {
    const auto = defaultEncerramentoResolver(
      evento({ payload: { terminalState: 'ENCERRADA', chatId: 'c', correlationId: 'k' } }),
    ) as EncerramentoAutomatico;
...
describe('GO-LIVE 12A · evento ENRIQUECIDO ⇒ registro COMPLETO sem consultas adicionais', () => {
  ...
  it('o painel apresenta EXATAMENTE os dados provenientes do evento', async () => {
    ...
    expect(painel.estrategiasMaisUtilizadas[0]).toEqual({
      chave: 'EST-CONSIG-CARTAO-RMC-001',
      ocorrencias: 1,
    });
    ...
    expect(persistido?.auditoria).toMatchObject({
      missionId: 'M-7',
      decisionId: 'dec-77777777',
      correlationId: 'corr-7',
      cliente: 'CLI-7',
