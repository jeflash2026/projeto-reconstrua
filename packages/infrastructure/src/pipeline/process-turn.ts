...
export class AutonomousTurnPipeline {
  ...
  async processTurn(input: TurnInput): Promise<TurnOutcome> {
    ...
    const snapshot = await medir(
      steps,
      'truth',
      async () => (await d.truth.load(input.chatId)) ?? emptySnapshot(input.chatId),
    );
    ...
    const raciocinio = await medir(
      steps,
      'strategic-reasoning',
      () => raciocinar(facts, d.strategyCatalog),
      (r) => `${String(r.hipoteses.length)} hipóteses`,
    );

    // 4) EXECUTIVE MIND — DECIDE: UMA StrategicDecision (ou null).
    const decision = await medir(
      steps,
      'executive-mind',
      () => deliberar(raciocinio),
      (r) => (r ? `${r.strategyRef} (${r.confidence})` : 'sem decisão'),
    );
    ...
    const missionResult = await medir(
      ...
      async () => {
        ...
        return missionIntents.length > 0
          ? d.mission.execute(input.missionFacts, missionIntents)
          : null;
      },
    ...
    const response = await medir(
      steps,
      'conversation',
      () => d.conversation.respond(outcome.intents, input.context),
      (r) => `${String(r.length)} mensagem(ns)`,
    );
