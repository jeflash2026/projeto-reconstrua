...
import {
  ...
  type EvoHttp,
  type EvoHttpResponse,
} from './evolution-instance-client.js';
...
class FakeEvoHttp implements EvoHttp {
  ...
  request(
    ...
    url: string,
    _h: Readonly<Record<string, string>>,
    body?: unknown,
...
describe('EvolutionInstanceClient — parse defensivo', () => {
  ...
  it('connect extrai o base64 do QR (em qrcode.base64)', async () => {
    const http = new FakeEvoHttp(() => ({
      status: 200,
      body: { qrcode: { base64: 'data:image/png;base64,AAA' } },
    }));
      ...
      body: [
        {
          ...
          ownerJid: `${OFFICIAL}@s.whatsapp.net`,
  ...
  it('logout/delete usam DELETE nos endpoints certos', async () => {
    ...
    await c.logout('x');
    await c.deleteInstance('x');
    expect(http.calls.map((k) => `${k.method} ${k.url}`)).toEqual([
      'DELETE https://evo.test/instance/logout/x',
      'DELETE https://evo.test/instance/delete/x',
    ]);
  });
});

function runtime(
  routes: (m: string, u: string) => EvoHttpResponse,
...
describe('WhatsAppConnectionRuntime', () => {
  it('createNew cria, configura webhook e persiste a instância como PENDENTE (número vazio)', async () => {
    const { rt, http, store } = runtime((_m, u) =>
      u.endsWith('/instance/create')
        ? { status: 201, body: { hash: 'K' } }
        : u.includes('/instance/connect/')
          ? { status: 200, body: { base64: 'QR' } }
          : { status: 200, body: {} },
    );
    ...
    expect(saved?.evolution).toMatchObject({
      instance: 'reconstrua-prod',
      apiKey: 'K',
      whatsappNumber: '',
    });
  });
  ...
  it('confirm com número DIVERGENTE → NÃO ativa e devolve o erro exato', async () => {
    const { rt, store } = runtime(() => ({
      status: 200,
      body: [{ name: 'x', ownerJid: `${WRONG}@s.whatsapp.net`, connectionStatus: 'open' }],
    }));
  ...
  it('confirm com instância ainda desconectada → erro de "não conectada"', async () => {
    const { rt } = runtime(() => ({
      status: 200,
      body: [{ name: 'x', ownerJid: null, connectionStatus: 'connecting' }],
    }));
    const r = await rt.confirm('x', { role: 'admin' });
    expect(r.connected).toBe(false);
  ...
  it('discard faz logout+delete e limpa a instância pendente', async () => {
    ...
    await store.save({
      ...
      evolution: { baseUrl: '', instance: 'velha', apiKey: 'k', whatsappNumber: WRONG },
    });
    await rt.discard('velha', { role: 'founder' });
    expect(http.calls.map((k) => k.method)).toEqual(['DELETE', 'DELETE']);
    expect((await store.load())?.evolution).toMatchObject({
      instance: '',
      apiKey: '',
      whatsappNumber: '',
    });
  ...
  it('GO-LIVE-06 (BUG 2): instância configurada não conectada → resolve pelo número e reporta o nome real', async () => {
    ...
    const { rt } = runtime(
      (_m, u) =>
        u.includes('/instance/fetchInstances')
          ? {
              status: 200,
              body: [
                {
...
describe('WhatsAppConnectionRuntime.diagnose — a causa real de cada dependência', () => {
  ...
  it('DNS: ENOTFOUND vira mensagem de host não resolvido', async () => {
    const throwing: EvoHttp = {
      request: () =>
        Promise.reject(Object.assign(new Error('fetch failed'), { cause: { code: 'ENOTFOUND' } })),
    };
    const report = await diagRuntime(throwing).diagnose();
  ...
  it('NÚMERO OFICIAL não conectado: marca falha e lista as instâncias encontradas', async () => {
    const http = new FakeEvoHttp(() => ({
      status: 200,
      body: [
        { name: 'outra-instancia', ownerJid: '5599@s.whatsapp.net', connectionStatus: 'open' },
      ],
    }));
    const inst = (await diagRuntime(http).diagnose()).steps.find((s) => s.step === 'Instância');
  ...
  it('BANCO indisponível: a exceção do Postgres vira o detalhe do passo (não engolida)', async () => {
    ...
    const rt = diagRuntime(http, {
      diagnostics: {
        baseUrl: 'https://evo.test',
        db: () => Promise.reject(new Error('connection to server failed')),
